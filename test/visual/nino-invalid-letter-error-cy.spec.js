...
test("Welsh NINO invalid letter validation error", async ({ page }) => {
  const rpPage = new RelyingPartyPage(page, "success");
  await rpPage.goto("Happy Harriet");

  const ninoPage = new NinoPage(page);

  await ninoPage.enterNINO("GB123456A"); // or whatever value passed in your English test
  ...
  await takeAndCompareScreenshots(page, "nino-invalid-letter-validation-cy");
});
